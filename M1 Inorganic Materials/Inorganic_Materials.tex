...
\begin{document}
    ...
    \begin{ex}
        ...
        \end{equation}
        for \(\theta\ge 90^\circ\) measured in degrees. Since the potential superexchange is much stronger than kinetic superexchange, we only need very small spatial overlap (via ligand) for the kinetic term to dominate --- in this case \(J=0\) when \(\theta=97.5\).
    \end{ex}
    ...
    \begin{equation}
        \chi T =0\,.
    \end{equation}
    If \(n\) is even, then we have all spins cancelled except one spin left out so \(S_{\text{T}}=S\) and
    \begin{equation}
