...
\usepackage{amsmath,geometry,amsfonts,array,makecell,enumitem,bm,esint,booktabs,multirow,mathtools,upgreek,amssymb,pgfplots, chemfig, tikzorbital,subcaption,adjustbox}
...
\begin{document}
    ...
    \begin{enumerate}
        \item Set up the Spin Hamiltonian. This essentially defines the problem that we want to solve. For the most general case, we set
        \begin{equation}
            \hat{H}=-2\sum_{i>j}J_{ij}\hat{\vb{S}}_i\hat{\vb{S}}_j\,,
        \end{equation}
        where \(\hat{\vb{S}}_i\) is the spin operator for particle \(i\), and \(J_{ij}\) is the coupling strength between spin \(i\) and spin \(j\): it's positive for ferromagnetic coupling and negative for antiferromagnetic coupling. The factor \(2\) is a matter of convention.
    ...
    \subsubsection{Two Interacting Spins}
    First we need to write down the spin Hamiltonian. 
