...
\begin{document}
    ...
    We use thermodynamic arguments to find conditions for the stability of a single phase. Consider a homogeneous phase in an isolated system with constant \(N\), \(V\) and \(E\) that is divided into two equal halves. At equilibrium, the entropy of the total system is at a maximum. Now consider transferring a small amount of energy \(\Delta E\) from one half of the system to the other. At equilibrium, the temperature in the two halves of the system is the same, and hence to linear order in \(\Delta E\), the entropy does not change. Consider the second order variation,
    \begin{equation}
    ...
    If the system is at some volume \(V^*\) with \(V_1\le V^*\le V_2\), the system would have lower Helmholtz energy if it is separated into the two phases than if it exists in either of the single phases. Define \(v\coloneqq V/N\), then
    \begin{equation}
        Nv^*=N_1v_1+(N-N_1)v_2\,.
    \end{equation}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[scale=0.9]
            ...
            \draw[step=1.0,xshift=-2.5cm,yshift=-2.5cm] (0,0) grid (5,5);
    ...
    Above the \textit{critical} (or \textit{Curie}) \textit{temperature} \(k_B T_c=Jz\), there is only one solution \(\eval{s}_0=0\): at high temperatures, the entropy dominates the free energy and so the spins are randomly distributed. The significance of the entropy as a driving force is diminished as the temperature is decreased, and below the Curie temperature, the net alignment of spins, which is energetically favourable, becomes dominant, and we get two extra solutions with non-zero mean magnetisation. Amongst all the three solutions, the \(\eval{s}_0=0\) solution is actually a maximum rather than a minimum so we will ignore it. In the absence of an external magnetic field, the system is equally likely to be magnetised in the up or the down spin configurations. This kind of phenomenon is known as \textit{spontaneous symmetry breaking}: we have a system symmetric with respect to \(\{s_i\}\leftrightarrow \{-s_i\}\), i.e. the energy of the system does not change when we flip all the spins at the same time. The ground states are two degenerate symmetry-related states with opposite spins. The system has to choose to reside in either one of the ground states: just like if you put a ball on the top of a hill, it will spontaneously roll down hill in an arbitrary direction, breaking the symmetry. This will be more apparent when we discuss Landau theory later.

    Moreover, if you fall in either of the free energy minima, it will be very difficult to get into the other: it involves flipping all the spins one by one, and doing so would require overcoming a huge free energy barrier. This makes it extremely unlikely for thermal fluctuations to cause the transitions between the two states. This type of behaviour is \textit{non-ergodicity}, and it is a huge problem in simulating statistical mechanical systems on a computer. You will investigate this in detail in C8: \textit{Computer Simulation Methods}. In short, being \textit{ergodic} means that a point (the state of the system in this case) will eventually visit all parts of the space (the phase space) as it evolves over time, in a uniform and random sense. Here, our system is non-ergodic, meaning that if start in the \(\eval{s}>0\) state, you will probably stay there for an extremely long time without knowing that there is also a degenerate \(\eval{s}<0\) minimum.
    ...
    \end{equation}
    The subscript `MF' in this expression stands for the mean field approximation. It is very important to bear in mind that the free energy that results from the mean-field approximation is in general not simply the free energy of the reference system with the effective field, but has an additional term  \(\eval{U_1-U_0}_{0}\) that depends on the difference between the two potentials measured for the system with the effective field.
    ...
    In higher dimensions, \(d\ge 2\), the crude features of the phase diagram, including the existence of a phase transition, given by mean field theory are essentially correct. We can examine this by comparing the phase transition temperature we obtained by mean field theory with exact (numerical) results.
    ...
    \begin{equation}
        k_B T_c=\frac{2J}{\ln(\sqrt{2}+1)}\approx 2.269J\,.
    \end{equation}
    ...
    The 3D Ising model, on the other hand, has not been solved analytically. We can still obtain a numerical result using high-precision simulation methods like Monte Carlo, which gives
    \begin{equation}
        k_B T_c\approx 4.5115 J
    \end{equation}
    for a cubic lattice, while our mean-field treatment gives
    \begin{equation}
        k_B T_c=Jz=6J\,.
    \end{equation}
    We see our mean field treatment gets better for higher spatial dimensions. In \cref{Fig:Ising_temperature}, we plot the mean-field and exact (numerical) Ising phase transition temperatures against spatial dimensions for (hyper)cubic lattices, and we see that numerical results asymptotes the mean-field result as the spatial dimension increases, justifying our claim that the mean field treatment is exact in \(d\to\infty\) dimension.
    ...
    \begin{figure}[ht!]
        ...
        \begin{tikzpicture}
            ...
            \begin{axis}[
                ...
                xlabel={Dimension \(\displaystyle d\)},
                xmin=0.7, xmax=7.3,
                xtick style={color=black},
                y grid style={darkgray176},
                ylabel={\(\displaystyle k_B T_c/J\)},
                ymin=-0.7, ymax=14.7,
                ytick style={color=black}
            ]
            \addplot [semithick, steelblue31119180, mark=x, mark size=1.5, mark options={solid}]
            table {%
            1 2
            2 4
            3 6
            ...
            \addlegendentry{Mean field}
            \addplot [semithick, darkorange25512714, mark=x, mark size=1.5, mark options={solid}]
            table {%
            1 0
            2 2.269
            3 4.5115
    ...
    \end{align}
    where \(m=\eval{s}\) is the mean magnetisation, \(m_0\) is the mean magnetisation at \(B=0\) and \(\chi\) is the magnetic susceptibility defined as
    \begin{equation}
    ...
    \end{align}
    Here \(\eval{-}_N\) denotes the canonical ensemble average over the configuration space of the \(N\)-particles. In other words, the excess chemical potential is related to the average of \(\eval{\exp(-\beta\Delta U_{N,N+1})}_N\) over all possible positions of the particle \(N+1\). In a translationally invariant system, such as a liquid or a gas, this quantity should not depend \(\vb{r}_{N+1}\), so we may write
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}
            ...
            \end{scope}
        \end{tikzpicture}
        \caption{Widom's insertion method to determine the excess chemical potential of a dilute non-ideal gas system.}
    \end{figure}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}
            ...
            \draw[step=0.6,thin] (0,0) grid (4.2,4.2);
    ...
    \begin{figure}
        \centering
        \begin{tikzpicture}
            \node at (-5,0){\tikz{\pgfmathsetseed{1234}
            \coordinate (p) at (0,0);
            \pgfmathsetmacro{\a}{360*rnd}

            \foreach \i in {1,...,100}{
                \draw[blue] (p) -- ++({\a}: rand*0.07 + 0.25) coordinate (p);
            ...
            \foreach \i in {1,...,100}{
                \draw[blue!50!cyan] (p) -- ++(360*rand: rand*0.07 + 0.25) coordinate (p);
            }}};
            ...
            \foreach \i in {1,...,100}{
                \draw[cyan] (p) -- ++({\a}: rand*0.07 + 0.25) coordinate (p);
    ...
    We can multiply both sides of the Langevin equation (\ref{Langevin}) by \(v(0)\) and use the assumption\footnote{Is the velocity really not correlated with the force? It is a good approximation since the force is random, but we will examine this more carefully in Part III M4: \textit{Energy Landscapes and Soft Matters.}} \(\eval{v_x(0)F_x^R(t)}=0\) to obtain the condition of the velocity correlation function
    \begin{equation}
    ...
    \begin{figure}
        \centering
        \begin{tikzpicture}
            \draw[step=0.25,black] (-0.15,-0.15) grid (3.15,3.15);
